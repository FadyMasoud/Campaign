...
| 6 | Sending is safe and honest. Confirmation count = what is approved. No double-send, no silent half-send. Past approvals still read as approved. | 5, 9 | ☑ |
...
| Where send progress/results are recorded | `/portal/sends/{id}`, `campaign_sends`, `send_recipients`, `send_batches` | ☑ |
| Provider key (given to graders) | From the brief email — held in `.env.local` | ☑ |
| Shared link + its password | Phase 7 — published, password `velocity-review-2026` | ☑ |
| 300-word note | `docs/SUBMISSION-NOTE.md` — 289 words | ☑ |
